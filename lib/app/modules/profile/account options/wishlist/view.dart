              ...
              itemBuilder: (context, index) {
                ...
                return InkWell(
                  onTap: () {
                    ...
                    // Convert wishlist product data to the format expected by ProductDetailsController
                    
                    // Format images with proper URLs
                    final formattedImages = images.map((img) {
                      if (img.startsWith('http')) {
                        return img;
                      }
                      final cleanPath = img.startsWith('/') ? img.substring(1) : img;
                      return '${AppUrls.baseImageUrl}$cleanPath';
                    }).toList();
                    
                    final productData = {
                      'id': productId,
                      '_id': productId, // Add MongoDB format ID as well
                      'name': productName,
                      'brand': brand,
                      'price': price,
                      'imageUrl': formattedImages.isNotEmpty ? formattedImages[0] : '',
                      'discount': discount,
                      'images': formattedImages,
                      // Add size and other product data
                      'sizeType': sizes,
                      // Add any other available product data from the original product
                      ...product,
                    };
                    
                    print('🔍 Wishlist navigation data: $productData');
                    print('📸 Formatted images: $formattedImages');
                    
                    Get.toNamed(Routes.productDetails, arguments: productData);
                  },
